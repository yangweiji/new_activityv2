          ...
          <div v-if="!canBuyVip" class="kind-list__item">
            ...
            <div class="weui-flex,kind-list__item-hd kind-list__item-hd_show c-text-center" v-if="item.communityUser.memberTime">
              <div class="weui-flex__item">
                <span style="color:#8a8a8a;">入会时间：{{item.communityUser.memberTime}}</span>
              </div>
            </div>
...
export default {
    ...
    payVip() {
      ...
      if (!that.isAgree) {
        wx.showModal({
          title: "会员提示",
          content:
            "申请成为会员前，请先阅读《相关条款》，明了会员的权利和义务，并同意遵守团体的相关规定。",
          confirmText: "确定",
          showCancel: false,
  ...
  onShow() {
    ...
    //接受参数
    this.communityId =
      this.$root.$mp.query.communityId || this.$root.$mp.query.scene;
