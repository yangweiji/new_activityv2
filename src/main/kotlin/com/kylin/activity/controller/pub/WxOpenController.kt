...
class WxOpenController {
    ...
    @GetMapping("/bindwx")
    fun bindWechat(request: HttpServletRequest, response: HttpServletResponse) {
        val state = UUID.randomUUID().toString()
        //保存至session中
        request.session.setAttribute("WECHAT_STATE", state)

        response.sendRedirect("https://open.weixin.qq.com/connect/qrconnect?appid=${wechatMpProperties!!.appId}&redirect_uri=${URLEncoder.encode(wechatOpenProperties!!.webWxBindUri, "UTF-8")}&response_type=code&scope=snsapi_login&state=$state#wechat_redirect")
    }
}