$(function () {
    ...
    var t = $('#bmTable')
        ...
        // serverSide: true,
        ajax: {
            "url": "/sec/thirdactivity/getAttendUsers",
    ...
    $.fn.dataTable.ext.buttons.check = {
        ...
        action: function (e, dt, node, config) {
            ...
            $.ajax({
                cache: true,
                type: "POST",
                url: '/sec/thirdactivity/checkrefund',
                data: JSON.stringify(d),// 指定请求的数据格式为json，实际上传的是json字符串
                contentType: 'application/json;charset=utf-8',//指定请求的数据格式为json,这样后台才能用@RequestBody 接受java bean
                dataType: "json",
                async: false,
        ...
        action: function (e, dt, node, config) {
            ...
            if (window.confirm("请确认删除？")) {
                $.ajax({
                    cache: true,
                    type: "POST",
                    url: '/sec/thirdactivity/deleteAttendUsers',
                    data: JSON.stringify(d),// 指定请求的数据格式为json，实际上传的是json字符串
                    contentType: 'application/json;charset=utf-8',//指定请求的数据格式为json,这样后台才能用@RequestBody 接受java bean
                    dataType: "json",
                    async: false,
